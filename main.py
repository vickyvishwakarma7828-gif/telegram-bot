...
# --- BROADCAST COMMANDS FOR ADMIN ---

# 1. Broadcast to ALL Users (/broadcast <message>)
@bot.message_handler(commands=['broadcast'])
def broadcast_all(message):
    clean_admin = ADMIN_TELEGRAM_USERNAME.replace("@", "").strip()
    if message.from_user.username and message.from_user.username.lower() != clean_admin.lower():
        bot.reply_to(message, "❌ Aapke paas is command ka access nahi hai.")
        return

    msg_text = message.text.replace("/broadcast", "").strip()
    if not msg_text:
        bot.reply_to(message, "⚠️ Message likhein!\n\n*Usage:* `/broadcast Aapka Message`", parse_mode="Markdown")
        return

    sent_count, failed_count = 0, 0
    bot.reply_to(message, f"📢 All-Users Broadcast Started...\nTotal Registered: {len(all_users)}")
...
# 2. Broadcast ONLY to Buyers (/buyer_broadcast <message>)
@bot.message_handler(commands=['buyer_broadcast'])
def broadcast_buyers(message):
    clean_admin = ADMIN_TELEGRAM_USERNAME.replace("@", "").strip()
    if message.from_user.username and message.from_user.username.lower() != clean_admin.lower():
        bot.reply_to(message, "❌ Aapke paas is command ka access nahi hai.")
        return

    msg_text = message.text.replace("/buyer_broadcast", "").strip()
    if not msg_text:
        bot.reply_to(message, "⚠️ Message likhein!\n\n*Usage:* `/buyer_broadcast Sirf Buyers ke liye message`", parse_mode="Markdown")
        return

    sent_count, failed_count = 0, 0
    bot.reply_to(message, f"🛒 Buyers-Only Broadcast Started...\nTotal Buyers: {len(buyer_users)}")
...
def callback_listener(call):
    ...
    if data == "btn_back":
        ...
        text = (
            f"🪙 *BINANCE PAY SYSTEM (USDT)* 🪙\n\n"
            f"Send USDT / Crypto via Binance Pay ID:\n\n"
            ...
            f"💡 *Note:* Payment auto-converted & directly credited to UPI: `{DEFAULT_UPI_ID}`"
        )
        markup = InlineKeyboardMarkup()
        ...
        text = (
            f"💰 *bKASH PAYMENT (BANGLADESH)* 🇧🇩\n\n"
            f"Send Taka to our bKash Agent/Personal number:\n\n"
            f"📱 **bKash Personal Number:** `{BKASH_NUMBER}`\n\n"
            f"📌 *Instructions:*\n"
            f"1. Use 'Send Money' option.\n"
            f"2. Send screenshot & TrxID to Admin 👉 @{clean_admin}\n\n"
            f"💡 *Note:* BD Taka auto-converted & credited to UPI: `{DEFAULT_UPI_ID}`"
        )
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("💬 Send Proof to Admin", url=f"https://t.me/{clean_admin}"))
        markup.add(InlineKeyboardButton("🖐 Back", callback_data="btn_balance"))
        ...
        text = f"{app_emoji} *PANEL - {app_title.upper()} PACKAGES*\n━━━━━━━━━━━━━━━━━━\n\n"
