...
def home_keyboard():
    ...
    kb.add(
        success_button("🧩 EXTRACT EMOJI", "extract"),
        primary_button("ℹ️ HELP", "help")
    )

    kb.add(
        danger_button("✖ CLOSE", "close")
    )
...
def start(message):
    ...
    text = (
        ...
        "🧩 Custom Emoji वाला message मुझे भेजो "
        "या forward करो।\n\n"
        "✨ मैं automatically Custom Emoji ID निकालूँगा।\n\n"
        "📋 हर ID के सामने अलग COPY button मिलेगा।"
    )
...
def help_command(message):
    ...
    text = (
        ...
        "4️⃣ COPY button से ID copy कर सकते हो।"
    )
...
def copy_button(emoji_id):
    ...
    try:
        ...
        # Fallback for older Telegram library versions
        return InlineKeyboardButton(
            text="📋 COPY ID",
...
def send_results(message, emoji_ids):
    ...
    for index, emoji_id in enumerate(unique_ids, start=1):
...
def handle_sticker(message):
    ...
    if not emoji_id:
        ...
        bot.reply_to(
            message,
            "❌ इस sticker में Custom Emoji ID नहीं मिली।"
        )
...
def callback_extract(call):
    ...
    bot.send_message(
        ...
        "🧩 अब Custom Emoji वाला message भेजो।"
    )
...
def callback_help(call):
    ...
    bot.send_message(
        ...
        (
            ...
            "हर ID के सामने 📋 COPY ID button मिलेगा।"
        )
    )
...
def home():
    ...
    return "Custom Emoji Extractor V3 is running.", 200
